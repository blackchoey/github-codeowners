...
export class OwnershipEngine {
  ...
  public static FromCodeownersFile(filePath: string) {
    try {
      const originalFile = fs.readFileSync(filePath).toString();
      console.log(originalFile);
      const file = originalFile.replace(/\r/g, '');
      console.log(file);
      ...
      for (const line of lines) {
        ...
        console.log(`ASIIC: ${stringToAsciiCodes(line)}`)
...
function stringToAsciiCodes(str: string) {
  const asciiCodes = [];
